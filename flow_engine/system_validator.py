...
class SystemValidator:
    ...
    def validate_system(self, quick_check: bool = False, auto_fix: bool = False) -> Dict[str, Any]:
        ...
        # 4.5. Action flow consistency validation
        self._validate_action_flow_consistency()
    ...
    def _check_file_structure(self):
        ...
        # Core required files (not configuration files)
        core_required_files = [
            'mindnext_hooks.py',
            'README.md', 
            'requirements.txt',
        ...
        # Dynamically check for any configuration files in config/
        config_files_found = []
        if (self.hooks_dir / "config").exists():
            config_files_found = list((self.hooks_dir / "config").glob("*.json")) + list((self.hooks_dir / "config").glob("*.toml"))
        ...
        # Report configuration files by category
        if config_files_found:
            rule_configs = [f.name for f in config_files_found if 'rules_config' in f.name]
            system_configs = [f.name for f in config_files_found if 'system' in f.name]
            other_configs = [f.name for f in config_files_found if 'rules_config' not in f.name and 'system' not in f.name]
            
            print(f"   ✅ Rule configurations: {rule_configs}")
            print(f"   ✅ System configurations: {system_configs}")
            if other_configs:
                print(f"   ✅ Other configurations: {other_configs}")
        else:
    ...
    def _validate_configurations(self):
        ...
        # Dynamically check any rules configuration files
        rules_configs_found = 0
        total_rules = 0
        
        for config_file in (self.config_dir).glob("rules_config*.json"):
            rules_configs_found += 1
            try:
                ...
                print(f"   ✅ {config_file.name}: {len(rules)} rules")
                    
            except Exception as e:
                self.issues.append({
                    'type': 'critical',
                    'category': 'configuration',
                    'message': f'Invalid JSON in {config_file.name}: {e}',
        ...
        for config_file in (self.config_dir).glob("*.toml"):
            ...
            try:
                ...
                if 'mappings' in config_data:
                    ...
                    print(f"   ✅ {config_file.name}: {len(mappings)} mappings")
                    
            except Exception as e:
                self.issues.append({
                    'type': 'critical', 
                    'category': 'configuration',
                    'message': f'Invalid TOML in {config_file.name}: {e}',
                    'file': str(config_file)
                })
        
        if rules_configs_found == 0:
            self.issues.append({
                'type': 'critical',
                'category': 'configuration',
                'message': 'No rules configuration files found'
            })
        elif total_rules < 5:
            self.warnings.append({
                'type': 'warning',
                'category': 'rules',
                'message': f'Only {total_rules} total rules found, recommend at least 5'
            })
    ...
    def _validate_action_flow_consistency(self):
        """Validate action_flow consistency between configs and registered actions"""
        print("🔗 Validating action flow consistency...")
        
        # Get registered action types from flow_coordinator
        registered_actions = self._get_registered_action_types()
        
        # Get action references from configuration files
        config_action_refs = self._extract_action_flow_references()
        
        # Check for inconsistencies
        inconsistencies = []
        missing_actions = []
        
        for config_file, action_refs in config_action_refs.items():
            for action_ref in action_refs:
                action_type = self._parse_action_type(action_ref)
                if action_type and action_type not in registered_actions:
                    inconsistencies.append({
                        'config_file': config_file,
                        'action_reference': action_ref,
                        'parsed_action_type': action_type,
                        'issue': f'Action type "{action_type}" not registered in flow_coordinator'
                    })
                    missing_actions.append(action_type)
        
        # Report findings
        if inconsistencies:
            print(f"   ❌ Found {len(inconsistencies)} action flow inconsistencies")
            
            # Group by missing action type
            missing_action_summary = {}
            for inconsistency in inconsistencies:
                action_type = inconsistency['parsed_action_type']
                if action_type not in missing_action_summary:
                    missing_action_summary[action_type] = []
                missing_action_summary[action_type].append({
                    'config_file': inconsistency['config_file'],
                    'reference': inconsistency['action_reference']
                })
            
            self.issues.append({
                'type': 'critical',
                'category': 'action_flow_consistency',
                'message': f'Action flow inconsistencies: {len(inconsistencies)} references to unregistered actions',
                'details': {
                    'missing_action_types': list(set(missing_actions)),
                    'registered_actions': list(registered_actions),
                    'inconsistencies_by_type': missing_action_summary,
                    'total_inconsistencies': len(inconsistencies)
                }
            })
            
            # Print detailed report
            print("   📋 Detailed inconsistency report:")
            for action_type, refs in missing_action_summary.items():
                print(f"      🚫 Missing: {action_type}")
                for ref in refs:
                    print(f"         📄 {ref['config_file']}: {ref['reference']}")
        else:
            print("   ✅ All action flow references are consistent")
            
        # Additional check: suggest potential fixes
        if missing_actions:
            suggestions = self._generate_action_fix_suggestions(missing_actions, registered_actions)
            if suggestions:
                self.warnings.append({
                    'type': 'warning',
                    'category': 'action_flow_suggestions',
                    'message': 'Suggested fixes for action flow inconsistencies',
                    'suggestions': suggestions
                })
    
    def _get_registered_action_types(self) -> Set[str]:
        """Extract registered action types from flow_coordinator.py"""
        registered_actions = set()
        
        flow_coordinator_path = self.flow_engine_dir / "flow_coordinator.py"
        if not flow_coordinator_path.exists():
            return registered_actions
            
        try:
            with open(flow_coordinator_path, 'r', encoding='utf-8') as f:
                content = f.read()
                
            # Parse import statements to find action classes
            import_pattern = r'from \.actions import \((.*?)\)'
            import_match = re.search(import_pattern, content, re.DOTALL)
            
            if import_match:
                import_content = import_match.group(1)
                # Extract class names
                class_names = re.findall(r'Action(\w+)', import_content)
                
                # Convert to action type names (lowercase)
                for class_name in class_names:
                    action_type = f"action.{class_name.lower()}"
                    registered_actions.add(action_type)
                    
            # Also look for direct registration in _register_modular_actions
            register_pattern = r'modular_actions = \[(.*?)\]'
            register_match = re.search(register_pattern, content, re.DOTALL)
            
            if register_match:
                register_content = register_match.group(1)
                additional_classes = re.findall(r'Action(\w+)\(\)', register_content)
            ...
            print(f"   ⚠️ Warning: Failed to parse flow_coordinator.py: {e}")
            
        return registered_actions
    
    def _extract_action_flow_references(self) -> Dict[str, List[str]]:
        """Extract action_flow references from all configuration files"""
        action_refs = {}
                ...
                for rule in rules:
                    ...
                    for action in actions:
                        if isinstance(action, str):
                            ...
                            refs.append(action['type'])
                            
                if refs:
                    action_refs[config_file.name] = refs
                    
            except Exception as e:
                print(f"   ⚠️ Warning: Failed to parse {config_file.name}: {e}")
    ...
    def _parse_action_type(self, action_reference: str) -> Optional[str]:
        """Parse action type from action reference string"""
        if not isinstance(action_reference, str):
            return None
            
        # Handle formats like:
        # "action.memory/operation=record"
        # "action.utility/operation=buffer.get&count=5"
        # "action.ai/operation=analyze"
        
        # Extract the action type (before the first '/' or '?')
        action_type_match = re.match(r'^(action\.\w+)', action_reference)
        if action_type_match:
            return action_type_match.group(1)
            
        return None
    
    def _generate_action_fix_suggestions(self, missing_actions: List[str], registered_actions: Set[str]) -> List[Dict[str, str]]:
        """Generate suggestions for fixing missing action references"""
        suggestions = []
        
        # Create mapping of missing to potential replacements
        action_mapping = {
            'action.memory': ['action.buffer', 'action.cache'],
            'action.utility': ['action.buffer', 'action.cache', 'action.notification'],
            'action.logging': ['action.notification'],
            'action.storage': ['action.buffer', 'action.cache']
        }
        
        for missing_action in missing_actions:
            if missing_action in action_mapping:
                potential_replacements = [
                    replacement for replacement in action_mapping[missing_action] 
                    if replacement in registered_actions
                ]
                
                if potential_replacements:
                    suggestions.append({
                        'missing_action': missing_action,
                        'suggested_replacements': potential_replacements,
                        'reason': f'"{missing_action}" has been refactored or renamed'
                    })
            else:
                # Check for similar names
                similar_actions = [
                    registered for registered in registered_actions
                    if registered.replace('action.', '').replace(missing_action.replace('action.', ''), '') != registered.replace('action.', '')
                ]
                
                if similar_actions:
                    suggestions.append({
                        'missing_action': missing_action,
                        'suggested_replacements': similar_actions[:3],  # Top 3 matches
                        'reason': 'Similar action types found'
                    })
